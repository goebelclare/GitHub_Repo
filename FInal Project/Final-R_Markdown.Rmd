...
First, let's select only the numerical values from our dataset.

```{r Q6.3.1, include=TRUE, echo=F}
final_df_numerical <- select_if(final_df, is.numeric)  
...
data.frame(variable = names(final_df_numerical),
           ...
           row.names = NULL) %>% 
kable(caption='Figure 6: Structure of final_df_numerical')
```
...
#kmeans(final_df, centers, nstart)
```
: *Figure 50: Fitting to a log(y) = (m)(log(x)) + b curve yields much stronger relationship.*
