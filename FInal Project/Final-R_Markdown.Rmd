...
First, let's create and index label.
...
final_df_cluster <- final_df_cluster %>% select(index, Deaths.Air.Pollution.per.100k, gdp.per.capita)
final_df_cluster = aggregate(final_df_cluster, by=list(index_=final_df_cluster$index), FUN=median)
final_df_cluster <- subset(final_df_cluster, select = -c(index))
...
From the total Within Sum of Squares plot, we find that the optimal number of *K* seems to be at 3 using the Elbow method which identifies the *K* number where the Total Sum of Squares begins to level off, or where adding additional *K* clusters only improves our model marginally.
...
From the Gap Statistic plot, we find that the optimal number of *K* seems to be at 10 with the highest gap statistic, however, we observed using the Elbow method that the Total Sum of Squares begins to level off from 3 to 10. We will stick with using *K* = 3.

#### Perform K-Means Clustering with Optimal *K*

We perform a K-Means clustering analysis with a *K* of 3 and here are the results below.
...
km <- kmeans(final_df_numerical_scaled, centers = 8, nstart = 25)
...
Visualize countries within each group
