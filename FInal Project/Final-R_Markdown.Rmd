...
```{r}
final_df_table1 = table(final_df$Population.thousands, final_df$gdp.per.capita, final_df$Deaths.Air.Pollution.per.100k)
xkabledply(final_df_table1, title = "Contingency tabel for Population vs GDP vs Deaths")
```

```{r}
chisqres1 = chisq.test(final_df_table1)
chisqres1
```

```{r}
pop_gdp_deathsLogit <- glm( ~ titanic_orig$age + titanic_orig$pclass + titanic_orig$sex, data = final_df)
```
```{r}
