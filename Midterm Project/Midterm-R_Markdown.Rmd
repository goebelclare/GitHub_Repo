...
First, let's just take a look at how the different defined regions are grouped.

```{r 4.6, include=TRUE, fig.align="center", echo=F}
plot(final_df_sf_regions)
```
: *Figure 24: Global Map of SDGRegions and SubRegions*
...
g1
```
: *Figure 25: Global Intensity Map of Deaths due to Air Pollution per 100k People, 1990 to 2017*

```{r 4.8.01, include=TRUE, fig.align="center", echo=F}
...
g2
```
: *Figure 26: Global Intensity Map of GDP per Capita by Countries, 1990 to 2017*

Looks like some inverse correlation between *gdp.per.capita* and *deaths.air.pollution.per.100k* where the really high values (yellow to grey) in one figure is not in the other. This hints at some inverse relationship between these 2 features. We will dig into this more later in the report.

```{r 4.8.1, include=TRUE, fig.align="center", echo=F}
g3 = ggplot() + geom_sf(data = final_df_sf, aes(fill = Population.thousands), color = 'black') + scale_fill_distiller(palette = "YlGnBu") + coord_sf(crs = st_crs(4283)) + labs(title = "Population (thousands) by Countries",
              caption = "Data Source: United Nations")

g3
```
: *Figure 27: Global Intensity Map of Population by Countries, 1990 to 2017*

Not surprised here that China and India lead population for the world.

We also have the capability to inspect particular regions or countries only. Here we are showing a couple of regions to explore visually.

```{r 4.9, include=TRUE, fig.align="center", echo=F}
g4 = ggplot() + geom_sf(data = final_df_sf[final_df_sf$SDGRegion == 'EASTERNANDSOUTH-EASTERNASIA' & final_df_sf$Year == 2017, ], aes(fill = Deaths.Air.Pollution.per.100k), color = 'black') + scale_fill_distiller(palette = "YlGnBu") + coord_sf(crs = st_crs(4283)) + labs(title = "Population (thousands) by Countries", subtitle = "East and South-Eastern Asia",
              caption = "Data Source: United Nations")

g4
```
: *Figure 28: Intensity Map of Deaths due to Air Pollution per 100k People in East and Southeastern Asia, 2017*

```{r 4.9.1, include=TRUE, fig.align="center", echo=F}
...
g5
```
: *Figure 29: Intensity Map of Deaths due to Air Pollution per 100k People in Sub-Saharan Africa, 2017*
...
```{r SQ1, include=TRUE, fig.align="center", echo=F}
...
```
: *Figure 30 and 31: Normality plots for Population and deaths per 100,000*


```{r SQ1.1, include=TRUE, fig.align="center", echo=F}
...
```
: *Figure 32: Correlation Plot for Population, Deaths per 100,000, ad GDP per Capita*
...
```{r SQ2, include=TRUE, fig.align="center", echo=F}
library("data.table")
library(scales)

#Aggregate data by total deaths by country
deathsbyyear_c <- group_by(.data = final_df,SDGRegion, Country.x)
totdeath_c <- summarize(.data = deathsbyyear_c, total = mean(Deaths.Air.Pollution.per.100k, na.rm = TRUE))
...
air1 <- ggplot(data = topdeaths_c,
      ...
      geom_point() +
      scale_x_continuous(label = comma, limits = c(0, 300), breaks = seq(0,300,50)) +
      scale_color_manual(values = c("firebrick1","lightpink1","green4", "steelblue")) 

air1 <- air1 + labs(title = "10 Countries with the Highest Average Deaths per 100,000",
                            subtitle = "Deaths due to Air Pollution, between 1990 - 2017", 
                            caption = "Data Source: Kaggle",
                            x = "Deaths per 100,000",
                            y = "") +
         theme_minimal() +
...
air2 <- ggplot(data = lowdeaths_c,
      ...
      geom_segment(data = lowdeaths_c,
               mapping = aes(x = 0, xend = total, y = reorder(Country.x, -total), yend = Country.x)) +
      geom_point() +
      scale_x_continuous(label = comma, limits = c(0, 30), breaks = seq(0,30,5)) +
      scale_color_manual(values = c("darkorange3","azure4", "dodgerblue4","gold1", "cyan1","green4")) 

air2 <- air2 + labs(title = "10 Countries with the Lowest Average Deaths per 100,000",
                            subtitle = "Deaths due to Air Pollution, between 1990 - 2017", 
                            caption = "Data Source: Kaggle",
                            x = "Deaths per 100,000",
                            y = "") +
...
#       height = 4)
```
: *Figure 33 and 34: Lollipop Graphs of the Top 10 Countries with the Lowest and Highest Average Deaths *

### 3. Which regions have the lowest and highest deaths due to air pollution?

```{r SQ3, include=TRUE, fig.align="center", echo=F}
library(scales)
...
deaths_bar <- ggplot(data = totdeath_reg[which(totdeath_reg$Year ==1990 | totdeath_reg$Year ==2017),], mapping = aes(x = total, y = SDGRegion, fill = Year), size = 1.2) +
              geom_bar(position="dodge", stat="identity") +
              scale_x_continuous(label = comma, limits = c(0, 7500), breaks = seq(0,7500,1000)) +
              scale_fill_brewer(palette = "Paired") 


deaths_bar <- deaths_bar + labs(title = "Total Deaths per 100,000 by Air Pollution, by Region",
                                            subtitle = "1990 - 2017", 
                                            caption = "Data Source: Kaggle",
                                            y = "",
            ...
            deaths_bar <- deaths_bar + theme(plot.title = element_text(color = "black", size = 12, face = "bold", hjust = 0),
                                         plot.subtitle = element_text(color = "black", size = 10, hjust = 0 ),
                                         plot.caption = element_text(color = "black", size =8, face = "italic", hjust =0))
            deaths_bar

```
: *Figure 35: Grouped Bar Chart of Total Deaths per 100,000, by Region in 1990 and 2017 *
...
```{r sq4, include=TRUE, fig.align="center", echo=F}
library(scales)
...
deaths_line <- deaths_line + labs(title = "Total Deaths per 1000,000 by Air Pollution",
                                            subtitle = "1990 - 2017", 
                                            caption = "Data Source: Kaggle",
              ...
              theme(axis.title = element_text(size = 8, face = "bold"),
                    ...
                    panel.background = element_blank(),
                    panel.grid.major.y = element_blank(),
                    axis.line.x = element_line(color = "dodgerblue"),
                    axis.ticks = element_line(color = "dodgerblue"),
                    axis.text = element_text(size = 6),
                    legend.position = "none",
                    plot.subtitle = element_text(size = 8),
                    plot.title = element_text(size = 10, margin = margin(b = 10)))
            
            deaths_line <- deaths_line + theme(plot.title = element_text(color = "dodgerblue", size = 12, face = "bold", hjust = 0),
                                         plot.subtitle = element_text(color = "dodgerblue", size = 10, hjust = 0 ),
                                         plot.caption = element_text(color = "dodgerblue", size =8, face = "italic", hjust =0))
            deaths_line

```
: *Figure 36: Line Chart of Total Deaths per 100,000 caused by Air Pollution from 1990 to 2017 *
...
```
: *Figure 36, 37, 38, 39: The distribution of Deaths per 100,000 caused by air pollution (total), indoor air pollution, outdoor particulate pollution, and outdoor ozone pollution from 1990 to 2017 *
...
```
: *Figure 40, 41, 42, 43: The Years with the Highest and Lowest Deaths per 100,000 caused by air pollution (total), indoor air pollution, outdoor particulate pollution, and outdoor ozone pollution from 1990 to 2017 * *
