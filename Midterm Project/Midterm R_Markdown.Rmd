...
deaths_line <- deaths_line + labs(title = "Deaths per 1000,000 due to Air Pollution",
                                            subtitle = "1990 - 2017", 
                                            caption = "Data Source: Kaggle",
...
Even looking at a each individual SDGRegion, their linear fits get better overall but are still not particularly strong with the highest being Australia/New Zealand and Europe at R^2^ of 0.56 and 0.55 respectively.
...
```
: *Fig XX: Linear models for each Year, deaths due to air pollution per 100k vs GDP per capita, 1990 - 2017.*
...
```{r 5.4, include=TRUE, fig.align="center", echo=F}
...
#print(vif(fit5))
```
R^2^ values for adding more features are `r summary(fit3)$r.squared`, `r summary(fit4)$r.squared`, and `r summary(fit5)$r.squared`.

```{r Q5.4.1, include=FALSE}
knitr::opts_chunk$set(warning = F, results = "show", message = F)
```
```{r Q5.4.2, include=TRUE, echo=F}
xkablevif(fit3, title='Fig XX: VIF of lm(log(Deaths.Air.Pollution.per.100k) ~ log(gdp.per.capita)*SubRegion+Year')
```

```{r Q5.4.3, include=TRUE, echo=F}
xkablevif(fit4, title='Fig XX: VIF of lm(log(Deaths.Air.Pollution.per.100k) ~ log(gdp.per.capita)*SubRegion+Year.')
```

```{r Q5.4.4, include=TRUE, echo=F}
xkablevif(fit5, title='Fig XX: VIF of lm(log(Deaths.Air.Pollution.per.100k) ~ log(gdp.per.capita)*Year.')
```
...
I will conduct 6 2-sample t-tests:
...
| Sample log graph                                          | [Link](https://saylordotorg.github.io/text_intermediate-algebra/s10-03-logarithmic-functions-and-thei.html)           |
: Fig X: References
