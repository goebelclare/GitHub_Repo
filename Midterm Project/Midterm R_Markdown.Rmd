...
```{r 4.1, results='markup', fig.align="center"}
#Deaths
death_hist <- ggplot() + 
...
```{r 4.2, results='markup', fig.align="center"}
...
```{r 4.3, results='markup', fig.align="center"}
...
```{r 4.4, results='markup', fig.align="center"}
...
```{r 4.5, results='markup', fig.align="center"}
...
```{r 4.6, results='markup', fig.align="center"}
plot(final_df_sf_regions)
```
: *Figure 11: Global Map of SDGRegions and SubRegions*

```{r 4.7, results='markup', fig.align="center"}
...
```{r 4.8, results='markup', fig.align="center"}
...
```{r 4.9, results='markup', fig.align="center"}
...
-Define PM2.5
- What levels are considered low, moderate, and high? 0-50 | 51-100 |101-150 - source?
...
### 2. Which countries had the highest and lowest average pollution exposure? How is this related to the region?
```{r SQ2, results='markup'}
library("data.table")
lowairq <- subset(final_df, AirQ >51 & Year == 2017)
str(lowairq)
lowairq_top <- lowairq[order(lowairq$AirQ, decreasing = TRUE), ]  # Top N highest values by group
lowairq_top <- data.table(lowairq_top, key = "Country.x")
lowairq_top <- lowairq_top[ , head(.SD, 50)]
str(lowairq_top)

air1 <- ggplot(data = lowairq_top,
             mapping = aes(x = AirQ, y = reorder(Country.x , AirQ), color = SDGRegion)) +
      geom_segment(data = lowairq_top,
               mapping = aes(x = 0, xend = AirQ, y = reorder(Country.x, AirQ), yend = Country.x)) +
      geom_point() +
...
air1 <- air1 + labs(title = "Top 50 Highest Pollution Exposure, by Country and Region",
                            subtitle = "Average Exposure to PM2.5 fine particles, 2017", 
                            caption = "Data Source: Air quality and health: Exposure to PM2.5 fine particles (OECD)",
...
highairq <-subset(final_df, AirQ <51 & Year == 2017)
str(highairq)

highairq_top <- highairq[order(highairq$AirQ, decreasing = FALSE), ]  # Top N lowest values by group
highairq_top <- data.table(highairq_top, key = "Country.x")
highairq_top <- highairq_top[ , head(.SD, 50)]
str(highairq_top)

air2 <- ggplot(data = highairq_top,
             mapping = aes(x = AirQ, y = reorder(Country.x , AirQ), color = SDGRegion)) +
      geom_segment(data = highairq_top,
               mapping = aes(x = 0, xend = AirQ, y = reorder(Country.x, AirQ), yend = Country.x)) +
      geom_point() +
          scale_x_continuous(limits = c(0, 150), breaks = seq(0,150,25)) +
          scale_color_manual(values = c("darkorange3", "firebrick1","lightpink1","green3","dodgerblue4","gold1", "mediumorchid1", "azure4","sienna3")) 

air2 <- air2 + labs(title = "Top 50 Lowest Pollution Exposure, by Country and Region",
                            subtitle = "Average Exposure to PM2.5 fine particles, 2017", 
                            caption = "Data Source: Air quality and health: Exposure to PM2.5 fine particles (OECD)",
                            x = "Pollution Exposure",
                            y = "") +
...
### 5. Are these trends in air pollution nuanced by Region or Country? 

```{r SQ5, results='markup'}
library(scales)
#Aggregate data by total deaths by year
...
deaths_line <- ggplot() +
              ...
              scale_y_continuous(label = comma, limits = c(0, 20000), breaks = seq(0,20000,2000)) 


deaths_line <- deaths_line + labs(title = "Deaths per 1000,000 by Air Pollution, by Region",
                                            subtitle = "1990 - 2017", 
                                            caption = "?",
                                            y = "air Pollution Induced Deaths",
                                            x = "") +
...
```{r 200, results='markup'}
str(air_pollution_df)
```
```{r 300, results='markup'}
# select what we need
air_pollution_dfneed <- subset(air_pollution_df, select = -c(Code))
```
```{r 400, results='markup'}
